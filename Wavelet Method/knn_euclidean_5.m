...
%% KNN REE, K=5, Ecuclidean.
features=EEG_REE';

cp1 = classperf(labels);
knn = knnclassify(features(itest,:),features(itrain,:),labels(itrain),5);
...
%% KNN REE, K=10, Euclidean.

cp2 = classperf(labels);
knn = knnclassify(features(itest,:),features(itrain,:),labels(itrain),10);
